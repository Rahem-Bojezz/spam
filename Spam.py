...
def mr_wibu():
    mr_tytyd = input(f"{B}Kerem spam pole!!! {putih}({G}y{W}/{Y}n{putih}){R}:{W} ")
    if mr_tytyd=="y" or mr_tytyd=="Y":
        time.sleep(5)
        put()
    if mr_tytyd=="n" or mr_tytyd=="N":
        sys.exit(f"{W}[{R}!{W}] Kaloar deri script{biru}....{W}")
        time.sleep(5)
...
def put():
    ...
    try:
        ...
        for i in range(int(power_python)):
            ...
            if "call" in jag:
                mengetik(f"{W}[{G}✓{W}] Ngerem Spam Call Ka No {Y}{mr_am}")
            else:
            ...
            if "Berhasil Ngab" in gas:
                ...
                mengetik(f"{W}[{G}✓{W}] Ngirem Spam Ka No {Y}{mr_am}")
            time.sleep(1)
            ...
            if "ok" in dekor2:
                mengetik(f"{W}[{G}✓{W}] Ngerem Spam Sms Ka No {Y}{mr_am}")
            else:
        ...
        sys.exit(f"{W}[{R}!{W}] Eror Cek Jaringanna Bekna Tretan{abu}....{abu}")
    except KeyboardInterrupt:
        sys.exit(f"{W}[{R}!{W}] Keluar!!{abu}...{W}")
    except ValueError:
        sys.exit(f"{W}[{Y}!{W}] Bekna cek asal masok aghi{abu}.....")
